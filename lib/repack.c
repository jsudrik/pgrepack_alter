...
extern Datum PGUT_EXPORT repack_alter_version(PG_FUNCTION_ARGS);
...
extern Datum PGUT_EXPORT repack_swap_with_schema(PG_FUNCTION_ARGS);
...
PG_FUNCTION_INFO_V1(repack_alter_version);
...
PG_FUNCTION_INFO_V1(repack_swap_with_schema);
...
static void swap_heap_or_index_files_with_schema(Oid r1, Oid r2);
...
Datum
repack_alter_version(PG_FUNCTION_ARGS)
{
	return CStringGetTextDatum("pgrepack_alter " LIBRARY_VERSION);
}
...
repack_trigger(PG_FUNCTION_ARGS)
{
	...
	sql = makeStringInfo();
	appendStringInfo(sql, "INSERT INTO repack_alter.log_%u(pk, row) "
	...
	appendStringInfo(sql, ")::repack_alter.pk_%u) END, $2)", relid);
...
repack_indexdef(PG_FUNCTION_ARGS)
{
	...
	if (concurrent_index)
		...
		appendStringInfo(&str, "%s index_%u ON repack_alter.table_%u USING %s (%s)%s",
...
repack_swap(PG_FUNCTION_ARGS)
{
	...
	execute_with_args(SPI_OK_SELECT,
		...
		" WHERE X.oid = $1"
		"   AND Y.oid = ('repack_alter.table_' || X.oid)::regclass",
	...
	execute_with_args(SPI_OK_SELECT,
		...
		"   AND I.indisvalid"
		"   AND Y.oid = ('repack_alter.index_' || X.oid)::regclass",
		1, argtypes, values, nulls);
...
/**
 * @fn      Datum repack_swap_with_schema(PG_FUNCTION_ARGS)
 * @brief   Swapping relfilenode of tables and relation ids of toast tables
 *          and toast indexes, with schema swapping for ALTER operations.
 *
 * repack_swap_with_schema(oid, relname)
 *
 * @param	oid		Oid of table of target.
 * @retval			None.
 */
Datum
repack_swap_with_schema(PG_FUNCTION_ARGS)
{
	Oid				oid = PG_GETARG_OID(0);
	const char	   *relname = get_quoted_relname(oid);
	const char	   *nspname = get_quoted_nspname(oid);
	...
	/* swap tables with schema. */
	swap_heap_or_index_files_with_schema(oid, oid2);
	CommandCounterIncrement();
...
repack_drop(PG_FUNCTION_ARGS)
{
	 ...
	 *
	 * Consider how a deadlock could occur - if the DROP TABLE repack_alter.log_%u
	...
	if (numobj > 1)
	{
		execute_with_format(
			SPI_OK_UTILITY,
			"DROP TABLE IF EXISTS repack_alter.log_%u CASCADE",
			oid);
		--numobj;
	}
	...
	if (numobj > 0)
	{
		execute_with_format(
			SPI_OK_UTILITY,
			"DROP TYPE IF EXISTS repack_alter.pk_%u",
			oid);
		--numobj;
	}
	...
	if (numobj > 0)
	{
		execute_with_format(
			SPI_OK_UTILITY,
			"DROP TABLE IF EXISTS repack_alter.table_%u CASCADE",
			oid);
		--numobj;
	}
...
/*
 * Schema-aware version of swap_heap_or_index_files that also swaps pg_attribute entries
 */
static void
swap_heap_or_index_files_with_schema(Oid r1, Oid r2)
{

	/* First swap the schema (pg_attribute entries) for tables */
	if (get_rel_relkind(r1) == RELKIND_RELATION && get_rel_relkind(r2) == RELKIND_RELATION)
	{
		...
		List		*r2_attrs = NIL;
		ListCell	*lc;
		CatalogIndexState attr_indstate;
		
		/* Open pg_attribute */
#if PG_VERSION_NUM >= 120000
		attrRelation = table_open(AttributeRelationId, RowExclusiveLock);
#else
		attrRelation = heap_open(AttributeRelationId, RowExclusiveLock);
#endif
		
		/* Collect user attributes for r1 (attnum > 0) */
		ScanKeyInit(&key[0], Anum_pg_attribute_attrelid, BTEqualStrategyNumber, 184, ObjectIdGetDatum(r1));
		scan = systable_beginscan(attrRelation, AttributeRelidNumIndexId, true, NULL, 1, key);
		while (HeapTupleIsValid(attrTuple = systable_getnext(scan)))
		{
			Form_pg_attribute attr = (Form_pg_attribute) GETSTRUCT(attrTuple);
			if (attr->attnum > 0)  /* Only user columns */
				r1_attrs = lappend(r1_attrs, heap_copytuple(attrTuple));
		}
		systable_endscan(scan);
		
		/* Collect user attributes for r2 (attnum > 0) */
		ScanKeyInit(&key[0], Anum_pg_attribute_attrelid, BTEqualStrategyNumber, 184, ObjectIdGetDatum(r2));
		scan = systable_beginscan(attrRelation, AttributeRelidNumIndexId, true, NULL, 1, key);
		while (HeapTupleIsValid(attrTuple = systable_getnext(scan)))
		{
			Form_pg_attribute attr = (Form_pg_attribute) GETSTRUCT(attrTuple);
			if (attr->attnum > 0)  /* Only user columns */
				r2_attrs = lappend(r2_attrs, heap_copytuple(attrTuple));
		}
		systable_endscan(scan);
		
		attr_indstate = CatalogOpenIndexes(attrRelation);
		
		/* Delete existing user attributes for both tables */
		ScanKeyInit(&key[0], Anum_pg_attribute_attrelid, BTEqualStrategyNumber, 184, ObjectIdGetDatum(r1));
		scan = systable_beginscan(attrRelation, AttributeRelidNumIndexId, true, NULL, 1, key);
		while (HeapTupleIsValid(attrTuple = systable_getnext(scan)))
		{
			Form_pg_attribute attr = (Form_pg_attribute) GETSTRUCT(attrTuple);
			if (attr->attnum > 0)
				CatalogTupleDelete(attrRelation, &attrTuple->t_self);
		}
		systable_endscan(scan);
		...
		/* Insert r2's attributes into r1 */
		foreach(lc, r2_attrs)
		{
			HeapTuple attr = (HeapTuple) lfirst(lc);
			Form_pg_attribute attrForm = (Form_pg_attribute) GETSTRUCT(attr);
			attrForm->attrelid = r1;
			CatalogTupleInsertWithInfo(attrRelation, attr, attr_indstate);
		}
		
		/* Insert r1's attributes into r2 */
		foreach(lc, r1_attrs)
		{
			HeapTuple attr = (HeapTuple) lfirst(lc);
			Form_pg_attribute attrForm = (Form_pg_attribute) GETSTRUCT(attr);
			attrForm->attrelid = r2;
			CatalogTupleInsertWithInfo(attrRelation, attr, attr_indstate);
		}
		
		CatalogCloseIndexes(attr_indstate);
		
#if PG_VERSION_NUM >= 120000
		table_close(attrRelation, RowExclusiveLock);
#else
		heap_close(attrRelation, RowExclusiveLock);
#endif
		
		list_free_deep(r1_attrs);
		list_free_deep(r2_attrs);
	}
	
	/* Now do the regular file swap */
	swap_heap_or_index_files(r1, r2);
}
