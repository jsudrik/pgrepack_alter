/*
 * pg_repack_alter.c: bin/pg_repack_alter.c
 *
...
static char				*alter_clause = NULL; /* ALTER TABLE clause to be validated and applied */
...
/*
 * Validate ALTER TABLE clause to ensure it contains only allowed operations
 * Supports case-insensitive input validation
 */
static bool
validate_alter_clause(const char *clause)
{
	char *clause_lower;
	bool is_valid = false;
	size_t i;

	if (!clause || strlen(clause) == 0)
		return false;

	/* Convert to lowercase for case-insensitive matching */
	clause_lower = pgut_strdup(clause);
	for (i = 0; clause_lower[i]; i++)
		clause_lower[i] = tolower(clause_lower[i]);

	/* Check for ALTER COLUMN operations */
	if (strstr(clause_lower, "alter column") != NULL)
	{
		/* Check for allowed ALTER COLUMN subclauses */
		if (strstr(clause_lower, "set data type") != NULL ||
			strstr(clause_lower, "set default") != NULL ||
			strstr(clause_lower, "set storage") != NULL)
		{
			is_valid = true;
		}
	}
	/* Check for ADD COLUMN operations */
	else if (strstr(clause_lower, "add column") != NULL)
	{
		/* ADD COLUMN with DEFAULT is allowed */
		if (strstr(clause_lower, "default") != NULL)
		{
			is_valid = true;
		}
		else
		{
			/* ADD COLUMN without DEFAULT is also allowed */
			is_valid = true;
		}
	}
...
main(int argc, char *argv[])
{
	...
	/* Validate ALTER clause if provided */
	if (alter_clause && !validate_alter_clause(alter_clause))
		ereport(ERROR, (errcode(EINVAL),
			errmsg("Invalid ALTER clause. Only the following operations are allowed (case-insensitive):\n"
	...
	/* Validate --alter option compatibility */
	if (alter_clause)
	{
		if (alldb)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --all (-a)")));
		if (schema_list.head)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --schema (-c)")));
		if (r_index.head)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --index (-i)")));
		if (only_indexes)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --only-indexes (-x)")));
		if (parent_table_list.head)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --parent-table (-I)")));
		if (!table_list.head)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("--alter requires --table (-t)")));
		if (tablespace)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --tablespace (-s)")));
		if (moveidx)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --moveidx (-S)")));
		if (orderby)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --order-by (-o)")));
		if (noorder)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --no-order (-n)")));
		if (jobs > 0)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --jobs (-j)")));
		if (exclude_extension_list.head)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --exclude-extension (-C)")));
		if (!analyze)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --no-analyze (-Z)")));
		if (no_kill_backend)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --no-kill-backend (-D)")));
		if (no_superuser_check)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --no-superuser-check (-k)")));
		if (apply_count != APPLY_COUNT_DEFAULT)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --apply-count")));
		if (switch_threshold != SWITCH_THRESHOLD_DEFAULT)
			ereport(ERROR, (errcode(EINVAL),
				errmsg("cannot specify --alter and --switch-threshold")));
	}
...
preliminary_checks(char *errbuf, size_t errsize){
	...
	res = execute_elevel("select repack_alter.version(), repack_alter.version_sql()",
		0, NULL, DEBUG2);
	if (PQresultStatus(res) == PGRES_TUPLES_OK)
	{
		const char	   *libver;
		...
		/* the string is something like "pgrepack_alter 1.0.0" */
		snprintf(buf, sizeof(buf), "pgrepack_alter %s", PROGRAM_VERSION);
...
is_requested_relation_exists(char *errbuf, size_t errsize){
	...
	appendStringInfoString(&sql,
		...
		" NOT EXISTS("
		"  SELECT FROM repack_alter.tables WHERE relid=to_regclass(given_t.r))"
		...
		if (num_parent_tables)
		{
			...
			for (cell = parent_table_list.head; cell; cell = cell->next)
			{
				...
				appendStringInfo(&sql,
								 "relid = ANY(repack_alter.get_table_and_inheritors($%d::regclass))",
								 iparam + 1);
				params[iparam++] = cell->val;
				if (cell->next)
					appendStringInfoString(&sql, " OR ");
...
apply_log(PGconn *conn, const repack_table *table, int count)
{
	...
	res = pgut_execute(conn,
					   "SELECT repack_alter.repack_apply($1, $2, $3, $4, $5, $6)",
					   6, params);
...
repack_one_table(repack_table *table, const char *orderby)
{
	 ...
	 * or worse, interfering with a still-running pg_repack_alter.
	 */
	...
	indexres = execute(
		"SELECT indexrelid,"
		" repack_alter.repack_indexdef(indexrelid, indrelid, $2, FALSE) "
	 ...
	 */
	res = execute("SELECT repack_alter.conflicted_triggers($1)", 1, params);
	if (PQntuples(res) > 0)
	{
		ereport(WARNING,
				(errcode(E_PG_COMMAND),
	...
	printfStringInfo(&sql, "SELECT repack_alter.disable_autovacuum('repack_alter.log_%u')", table->target_oid);
	...
	temp_obj_num++;
	printfStringInfo(&sql, "SELECT repack_alter.disable_autovacuum('repack_alter.table_%u')", table->target_oid);
	...
	printfStringInfo(&sql, "SELECT 'repack_alter.table_%u'::regclass::oid",
					 table->target_oid);
	 ...
	 */
	printfStringInfo(&sql, "LOCK TABLE repack_alter.table_%u IN ACCESS EXCLUSIVE MODE",
					 table->target_oid);
	...
	params[0] = utoa(table->target_oid, buffer);
	pgut_command(conn2, "SELECT repack_alter.repack_swap($1)", 1, params);
	...
	command("SELECT repack_alter.repack_drop($1, $2)", 2, params);
	
	/* Apply ALTER clause to original table after cleanup if provided */
	if (alter_clause)
	{
		StringInfoData alter_sql;
		initStringInfo(&alter_sql);
		appendStringInfo(&alter_sql, "ALTER TABLE %s %s", table->target_name, alter_clause);
		elog(INFO, "Applying ALTER to original table: %s", alter_sql.data);
		command(alter_sql.data, 0, NULL);
		elog(INFO, "ALTER applied successfully to original table");
		termStringInfo(&alter_sql);
	}
 ...
 * with each other or a full-table repack_alter.
 */
static bool advisory_lock(PGconn *conn, const char *relid)
{
	PGresult	   *res = NULL;
...
repack_table_indexes(PGresult *index_details)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if (isvalid[0] == 't')
		{
			...
			res = execute("SELECT repack_alter.repack_indexdef($1, $2, $3, true)", 3,
						  params);
	...
	for (i = 0; i < num; i++)
	{
		...
		{
			params[0] = utoa(index, buffer[0]);
			pgut_command(connection, "SELECT repack_alter.repack_index_swap($1)", 1,
						 params);
		}
		else
...
repack_all_indexes(char *errbuf, size_t errsize)
{
	...
	{
		appendStringInfoString(&sql,
			"SELECT repack_alter.oid2text(i.oid), idx.indexrelid, idx.indisvalid, idx.indrelid, repack_alter.oid2text(idx.indrelid), n.nspname"
	...
	{
		appendStringInfoString(&sql,
			"SELECT repack_alter.oid2text(i.oid), idx.indexrelid, idx.indisvalid, idx.indrelid, $1::text, n.nspname"
			" FROM pg_index idx JOIN pg_class i ON i.oid = idx.indexrelid"
			" JOIN pg_namespace n ON n.oid = i.relnamespace"
		...
		for (cell = parent_table_list.head; cell; cell = cell->next)
		{
			...
			res = execute_elevel("SELECT quote_ident(n.nspname) || '.' || quote_ident(c.relname)"
								 ...
								 " WHERE c.oid = ANY (repack_alter.get_table_and_inheritors($1::regclass))"
								 "   AND c.relkind = 'r'"
...
pgut_help(bool details)
{
	...
	printf("      --alter=CLAUSE                 ALTER TABLE clause to apply (only with --table, --dry-run, --wait-timeout)\n");
}
